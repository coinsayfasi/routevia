...
function parseArgs(argv) {
  const args = { province: 'izmir', maxPerDistrict: 16, radiusM: 22000 };
  for (let i = 2; i < argv.length; i += 1) {
    const a = argv[i];
    if (a.startsWith('--province=')) args.province = a.slice(11);
    else if (a === '--province') args.province = argv[++i] || args.province;
    else if (a.startsWith('--max-per-district=')) args.maxPerDistrict = Number(a.slice(19)) || args.maxPerDistrict;
...
function mapCategory(types) {
  ...
  if (t.has('tourist_attraction') || t.has('monument')) return 'historical';
...
function tipsFor(category, districtName, provinceName) {
  const base = {
    nature: [`${provinceName} doğa rotasında öne çıkan durak.`, 'Bölgenin doğal yapısı korunmaktadır.', `${districtName} çevresinde yemek planını önceden yap.`, 'Sabah saatlerinde daha sakin deneyim sunar.'],
    beach: [`${provinceName} kıyı hattında popüler deniz noktası.`, 'Bölge yaz sezonunda yoğun ziyaret alır.', `${districtName} sahilinde yeme içme seçenekleri değişkendir.`, 'Gün batımı saatinde manzara güçlenir.'],
    historical: [`${provinceName} tarih rotasının güçlü duraklarından biri.`, 'Bölge antik ve Osmanlı dönem izleri taşır.', `${districtName} merkezinde yerel mutfak alternatiflerini dene.`, 'Kalabalıktan kaçmak için erken saatleri seç.'],
    viewpoint: ['Şehir ve çevre manzarası sunan seyir noktası.', 'Bölge fotoğraf rotalarıyla öne çıkar.', 'Kısa mola için yakında kafe alternatifi olabilir.', 'Gün batımından önce konumlan.'],
    ...
    lodging: ['Konaklama odaklı güçlü durak.', 'Turizm sezonunda doluluk yüksek olabilir.', 'Yakın çevrede yeme içme noktalarını kontrol et.', 'Check-in saatlerini önceden planla.'],
...
async function main() {
  ...
  const provinceSlug = slugifyTr(args.province);
  const pRows = await fetchJson(`${supabaseUrl}/rest/v1/provinces?select=id,name,slug&slug=eq.${provinceSlug}`, {
    headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` },
  });
  if (!Array.isArray(pRows) || pRows.length === 0) throw new Error(`Province not found: ${provinceSlug}`);
  ...
  if (!Array.isArray(districts) || districts.length === 0) throw new Error(`No districts for ${province.name}`);

  const queries = [
    'tourist attractions', 'museum', 'historical places', 'beach', 'viewpoint',
    'nature park', 'waterfall', 'canyon', 'restaurant', 'cafe', 'hotel',
  ];
  ...
  for (const d of districts) {
    ...
    for (const q of queries) {
      const body = {
        textQuery: `${q} in ${districtName}, ${province.name}, Turkey`,
        languageCode: 'tr',
        pageSize: 20,
      ...
      try {
        ...
        for (const p of (json.places || [])) {
          ...
          if (id && !collected.has(id)) collected.set(id, p);
        }
      } catch (_) {}
    ...
    for (const p of top) {
      ...
      const key = `${provinceSlug}:${slug}`;
      if (seen.has(key)) continue;
      seen.add(key);

      const tips = tipsFor(category, districtName, province.name);
  ...
  const lines = [header, ...rows.map((r) => [
    esc(r.name), esc(r.province), esc(r.district), esc(r.category), esc(r.lat), esc(r.lng),
    esc(r.price_level), esc(r.duration_min), esc(r.tags), esc(r.short_desc), esc(r.history_tip), esc(r.eat_tip), esc(r.pro_tip),
  ].join(','))];
  fs.writeFileSync(outPath, `${lines.join('\n')}\n`, 'utf8');
  console.log(`Generated ${province.name} batch rows=${rows.length}, requests=${reqCount}, file=${outPath}`);
}
