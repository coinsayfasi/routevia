...
-- 2) RLS disabled on public.spatial_ref_sys
-- 3) Function search_path mutable warnings

BEGIN;

-- 1) Force listed public views to SECURITY INVOKER where available.
DO $$
DECLARE
  v_name text;
BEGIN
...
-- 2) Enable RLS on PostGIS public table flagged by advisor.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
-- 3) Stabilize search_path for all functions in public schema.
DO $$
DECLARE
  r record;
BEGIN
