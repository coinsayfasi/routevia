...
begin
  case lower(coalesce(p_category, ''))
    when 'museum' then return 'museum';
    when 'historical' then return 'historical';
    when 'nature' then return 'nature';
    when 'beach' then return 'beach';
    when 'viewpoint' then return 'viewpoint';
    when 'market' then return 'market';
    when 'cafe' then return 'cafe';
    when 'food' then return 'food';
    when 'activity' then return 'activity';
    when 'waterfall' then return 'nature';
    when 'canyon' then return 'nature';
    when 'lake' then return 'nature';
    when 'national_park' then return 'nature';
    when 'hiking_trail' then return 'nature';
    when 'historical_site' then return 'historical';
    when 'archeological_site' then return 'historical';
    when 'castle_fortress' then return 'historical';
    when 'food_restaurant' then return 'food';
    when 'dessert' then return 'food';
    when 'street_food' then return 'food';
    when 'tour' then return 'activity';
    when 'tour_city' then return 'activity';
    when 'tour_boat' then return 'activity';
    when 'activity_family' then return 'activity';
    when 'activity_adventure' then return 'activity';
    when 'marina' then return 'activity';
    when 'nightlife' then return 'activity';
...
begin
  with src as (
    ...
    select
      l.clean_place_id,
      left(coalesce(pd.history_bullets, '{}'::text[]), 3),
      left(coalesce(pd.eat_drink_bullets, '{}'::text[]), 3),
      left(coalesce(pd.tips_bullets, '{}'::text[]), 4)
    from linked l
    ...
    select
      ...
      case
        when lower(coalesce(pm.source_kind, pm.source, 'curated')) in ('user_upload','curated','placeholder','open_license')
          then lower(coalesce(pm.source_kind, pm.source, 'curated'))
        else 'curated'
...
grant execute on function public.sync_curated_archive_to_clean(int, int) to authenticated;
